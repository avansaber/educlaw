"""Shared helper functions for EduClaw Scheduling unit tests."""
import argparse
...
ROOT_DIR = os.path.dirname(MODULE_DIR)             # educlaw-scheduling/
...
def run_init_db(db_path: str):
    spec = importlib.util.spec_from_file_location("init_db_scheduling", VERTICAL_INIT_PATH)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    mod.create_educlaw_scheduling_tables(db_path)
...
def seed_room(conn, company_id: str) -> str:
    ...
    conn.execute(
        """INSERT INTO educlaw_room
           ...
           VALUES (?, 'R101', 'classroom', 30, 'Main Building',
                   1, ?, '')""",
        (rid, company_id))
    conn.commit()
    return rid
...
def seed_instructor(conn, company_id: str) -> str:
    iid = str(uuid.uuid4())
    ns_val = f"INST-{iid[:8]}"
    conn.execute(
        """INSERT INTO educlaw_instructor
           (id, naming_series, name, email,
            is_active, company_id, created_by)
           VALUES (?, ?, 'Test Teacher', 'teacher@school.edu', 1, ?, '')""",
        (iid, ns_val, company_id))
    conn.commit()
    return iid
...
def seed_course(conn, company_id: str) -> str:
    ...
    conn.execute(
        """INSERT INTO educlaw_course
           ...
           VALUES (?, ?, ?, 'Intro to CS', 1, ?, '')""",
        (crs_id, code, code, company_id))
    conn.commit()
    return crs_id
