...
ROOT_DIR = os.path.dirname(MODULE_DIR)             # educlaw-highered/
...
def run_init_db(db_path: str):
    spec = importlib.util.spec_from_file_location("init_db_highered", VERTICAL_INIT_PATH)
...
def seed_student(conn, company_id: str) -> str:
    ...
    conn.execute(
        """INSERT INTO educlaw_student
           (id, naming_series, student_id, first_name, last_name, full_name, name,
            date_of_birth, email, status, company_id, created_by)
           VALUES (?, ?, ?, 'Test', 'Student', 'Test Student', 'Test Student',
                   '2005-01-01', 'test@univ.edu', 'active', ?, '')""",
        (sid, ns_val, sid, company_id))
    conn.commit()
    return sid
