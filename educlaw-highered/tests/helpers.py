...
def seed_course(conn, company_id: str, code: str = "CS101",
    ...
    """, (cid, code, name, credits, company_id, _now()))
    conn.commit()
    return cid
...
def seed_section(conn, course_id: str, company_id: str,
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?, ?, 'Dr. Smith', ?, 0, 'MWF 10:00', 'Room 101',
                'open', ?, ?)
    """, (sid, course_id, term, year, capacity, company_id, _now()))
    conn.commit()
    return sid
...
def seed_student_record(conn, company_id: str, program_id: str,
    ...
    ns_val = f"HSTU-{rid[:8]}"
    conn.execute("""
        INSERT INTO highered_student_record
...
def seed_faculty(conn, company_id: str, name: str = "Dr. Johnson",
    ...
    """Insert a test faculty member and return its ID."""
    fid = str(uuid.uuid4())
    ns_val = f"HFAC-{fid[:8]}"
    conn.execute("""
