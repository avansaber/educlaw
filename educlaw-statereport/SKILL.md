...
The `--ods-base-url` and `--oauth-token-url` parameter values are stored in the local
database only. The skill does not fetch or contact any external URL during configuration,
data entry, validation, or snapshot operations. No network call is made by any data-entry,
validation, snapshot, or submission-tracking action — all data is stored locally in the
EduClaw database.

The Ed-Fi sync actions (`submit-student-to-edfi`, `submit-enrollment-to-edfi`,
`submit-attendance-to-edfi`, `submit-sped-to-edfi`, `submit-el-to-edfi`,
`submit-discipline-to-edfi`, `submit-staff-to-edfi`, `submit-failed-syncs`,
`get-edfi-connection-test`) make outbound HTTPS calls to the configured ODS endpoint
**only when explicitly invoked** by the LEA administrator. These actions are the sole
source of external network activity.

**Credential handling:** The `oauth_client_secret` field is encrypted at rest using
AES-256 before database insertion. The `get-edfi-config` action does not return the
decrypted secret. No credential is written to logs, stdout, or error messages.
