"""Shared pytest fixtures for EduClaw SPED unit tests.
...
def db_path(tmp_path):
    """Per-test fresh SQLite database with full educlaw-sped schema."""
    path = str(tmp_path / "test.sqlite")

    # Step 1: Bootstrap foundation tables (company, naming_series, audit_log)
    bootstrap_foundation(path)

    # Step 2: Run educlaw-sped init_db (creates sped_* tables)
    run_init_db(path)
